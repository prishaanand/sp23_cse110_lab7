...
function initializeServiceWorker() {
  ...
  if ('serviceWorker' in navigator){
    ...
    window.addEventListener('load', () => {
      ...
      if(registration){
        console.log("Registration of ServiceWorker succeded.");
      }
      // B5. TODO - In the event that the service worker registration fails, console
      //            log that it has failed.
      else{
        console.error('Registration of ServiceWorker failed'); 
      }
  ...
  return new Promise(async (resolve, reject) => {
    ...
    for(const recipe of RECIPE_URLS){
      ...
      try {
        ...
        //            "await" again
        const recipeCurr = await fetchR.json(); 
        
        // A8. TODO - Add the new recipe to the recipes array
        recipeFetch.push(recipeCurr);
        ...
        //            resolve() method.
        if(recipeFetch.length == RECIPE_URLS.length){
